...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 d-flex justify-content-between align-items-center">
            <h6 class="m-0 font-weight-bold text-primary">Daftar Gedung (<?= number_format($total) ?> data)</h6>
            <a href="<?= base_url('/master/gedung/new') ?>" class="btn btn-primary btn-sm"><i class="bi bi-plus-lg"></i> Tambah Gedung</a>
        </div>
        <div class="card-body">
            <form method="get" action="<?= base_url('/master/gedung') ?>" class="mb-3">
                <div class="row g-2 align-items-end">
                    <div class="col-md-4">
                        <input type="text" name="q" class="form-control" value="<?= esc($params['q'] ?? '') ?>" placeholder="Cari kode, nama gedung, atau unit kerja...">
                    </div>
                    <div class="col-md-3">
                    ...
                    <div class="col-md-auto">
                        ...
                        <?php if (!empty(array_filter($params ?? []))): ?>
                            <a href="<?= base_url('/master/gedung') ?>" class="btn btn-secondary">Reset</a>
                        <?php endif; ?>
                    </div>
                </div>
            </form>
            ...
            <div class="table-responsive">
                <table class="table table-bordered" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><?= esc($g['kode_gedung']) ?></td>
                            <td><?= esc($g['nama_gedung']) ?></td>
                            <td><?= esc($g['nama_unit'] ?? '-') ?></td>
                            <td><?= esc($g['jumlah_lantai'] ?? '-') ?></td>
                            <td>
                                <?php if ($g['is_active']): ?>
                            ...
                            <td>
                                <div class="action-btn-group">
                                    <a href="<?= base_url('/master/gedung/' . $g['gd_id']) ?>" class="icon-btn detail-btn" title="Detail">
                                        <svg viewBox="0 0 24 24"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z" /><circle cx="12" cy="12" r="3" /></svg>
                                    </a>
                                    <a href="<?= base_url('/master/gedung/' . $g['gd_id'] . '/edit') ?>" class="icon-btn edit-btn" title="Edit">
                                        <svg viewBox="0 0 24 24"><path d="M3 21h18" /><path d="M14.7 5.3l4 4L8 20H4v-4L14.7 5.3z" /><path d="M13 6l4 4" /></svg>
                                    </a>
                                    <form action="<?= base_url('/master/gedung/' . $g['gd_id']) ?>" method="post" style="display:inline;">
                                        <?= csrf_field() ?>
                                        <input type="hidden" name="_method" value="DELETE">
                                        <button type="submit" class="icon-btn delete-btn" title="Hapus" onclick="return confirm('Yakin hapus gedung ini?')">
            ...
            $baseUrl = base_url('/master/gedung');
            ...
            <div class="text-center text-muted small">
                Menampilkan <?= count($gedung) ?> dari <?= $total ?> data
            </div>
        </div>
    </div>
</div>
